...
struct DecisionsPageDTO: Codable, Sendable { let items: [SessionDecisionDTO]; let pagination: RealSessionsPaginationDTO; var nullConditionEntries: Int { items.reduce(0) { $0 + $1.nullConditionEntries } } }
struct SessionDecisionDTO: Codable, Sendable { let id: String; let createdAt: Date; let decision: String?; let decisionLabel: String?; let score: DecimalString?; let scoreMin: DecimalString?; let price: DecimalString?; let controller: String?; let blockers: [String]?; let buyConditions: [String]?; let sellConditions: [String]?; let advice: String?; let summaryTitle: String?; let summaryMessage: String?; let nullConditionEntries: Int; enum CodingKeys: String, CodingKey { case id, decision, score, price, controller, blockers, advice; case createdAt = "created_at", decisionLabel = "decision_label", scoreMin = "score_min", buyConditions = "buy_conditions", sellConditions = "sell_conditions", summaryTitle = "summary_title", summaryMessage = "summary_message" }
    init(from decoder: Decoder) throws { let c = try decoder.container(keyedBy: CodingKeys.self); id = try c.decode(String.self, forKey: .id); createdAt = try c.decode(Date.self, forKey: .createdAt); decision = try c.decodeIfPresent(String.self, forKey: .decision); decisionLabel = try c.decodeIfPresent(String.self, forKey: .decisionLabel); score = try c.decodeIfPresent(DecimalString.self, forKey: .score); scoreMin = try c.decodeIfPresent(DecimalString.self, forKey: .scoreMin); price = try c.decodeIfPresent(DecimalString.self, forKey: .price); controller = try c.decodeIfPresent(String.self, forKey: .controller); blockers = try c.decodeLossyStringArrayIfPresent(forKey: .blockers); buyConditions = try c.decodeLossyStringArrayIfPresent(forKey: .buyConditions); sellConditions = try c.decodeLossyStringArrayIfPresent(forKey: .sellConditions); advice = try c.decodeIfPresent(String.self, forKey: .advice); summaryTitle = try c.decodeIfPresent(String.self, forKey: .summaryTitle); summaryMessage = try c.decodeIfPresent(String.self, forKey: .summaryMessage); nullConditionEntries = c.countNilStringArrayEntries(forKey: .buyConditions) + c.countNilStringArrayEntries(forKey: .sellConditions) + c.countNilStringArrayEntries(forKey: .blockers) }
}
...
extension KeyedDecodingContainer {
    func countNilStringArrayEntries(forKey key: Key) -> Int {
        guard contains(key), var container = try? nestedUnkeyedContainer(forKey: key) else { return 0 }
        var count = 0
        while !container.isAtEnd {
            if (try? container.decodeNil()) == true { count += 1 } else { _ = try? container.decode(String.self) }
        }
        return count
    }
}
