...
extension RealOrderDTO { func mapped(currency: String) -> TradingOrderSummary { TradingOrderSummary(id: id ?? krakenOrderID ?? UUID().uuidString, side: side ?? "—", status: OrderStatus(backend: status), message: "Ordre \(side ?? "") \(status ?? "") sur Kraken.".trimmingCharacters(in: .whitespaces), requestedQuantity: requestedQuantity?.value, executedQuantity: executedQuantity?.value, limitPrice: limitPrice.money(currency), averageExecutionPrice: averageExecutionPrice.money(currency), createdAt: createdAt, updatedAt: updatedAt) } }
