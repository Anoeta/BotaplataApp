...
final class PushNotificationsStore {
    ...
    private func withReplay<T: Sendable>(_ work: @escaping @Sendable (String) async throws -> T) async throws -> T { do { return try await authSession.withAccessTokenReplay(work) } catch let error as AuthenticationError where error == .accessTokenExpired { appState.markExpired(); throw error } catch let error as AuthenticationError where error == .deviceRevoked { appState.markRevoked(); throw error } catch { throw error } }
