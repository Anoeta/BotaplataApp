import Foundation
protocol PushNotificationsCache: Sendable { func load() async -> PushNotificationsCacheSnapshot?; func save(_ snapshot: PushNotificationsCacheSnapshot) async; func purge() async }
struct PushNotificationsCacheSnapshot: Codable, Equatable, Sendable { let notifications: [RealNotificationItem]; let summary: RealNotificationSummary?; let preferences: PushPreferences?; let savedAt: Date }
struct FilePushNotificationsCache: PushNotificationsCache { private let url: URL; private let version = 1; init(directory: URL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first ?? FileManager.default.temporaryDirectory) { url = directory.appendingPathComponent("botaplata-real-notifications-cache-v1.json") }
    ...
    private struct Box: Codable { let version: Int; let snapshot: PushNotificationsCacheSnapshot }
}
