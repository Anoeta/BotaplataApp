...
struct RemoteAuthenticationRepository: AuthenticationRepository {
    ...
    func logout(accessToken: String?) async { guard let accessToken else { return }; _ = try? await client.send(APIEndpoint(method: .post, path: "\(prefix)/logout", headers: HTTPHeaders.bearer(accessToken))) as EmptyResponse }
    func authorizedDevices(accessToken: String) async throws -> [AuthorizedDevice] { try map { try await client.send(APIEndpoint(method: .get, path: "\(prefix)/devices", headers: HTTPHeaders.bearer(accessToken))) } }
    func revokeDevice(id: String, accessToken: String) async throws -> DeviceRevocationResult { try map { try await client.send(APIEndpoint(method: .delete, path: "\(prefix)/devices/\(id)", headers: HTTPHeaders.bearer(accessToken))) } }
