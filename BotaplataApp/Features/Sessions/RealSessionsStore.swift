...
@MainActor @Observable final class RealSessionsStore {
    ...
    private var refreshTask: Task<Void, Never>?; private var nextTask: Task<Void, Never>?; private var detailTasks: [String: Task<Void, Never>] = [:]; private var generation = 0; private var detailGenerations: [String: Int] = [:]; private var detailLoadedAt: [String: Date] = [:]; private let detailCacheTTL: TimeInterval = 60
    ...
    func purge() async { refreshTask?.cancel(); nextTask?.cancel(); detailTasks.values.forEach { $0.cancel() }; content = .idle; pagination = nil; warnings = []; details = [:]; detailLoadedAt = [:]; await cache.purge() }
    ...
    func loadDetail(id: String, force: Bool = false) async { if let t = detailTasks[id] { BotaplataLog.sessions.info("RealSessionsStore.loadDetail session=\(id, privacy: .public) skipped reason=singleFlight"); await t.value; return }; if !force, visibleDetail(id) != nil, let loadedAt = detailLoadedAt[id], Date().timeIntervalSince(loadedAt) < detailCacheTTL { BotaplataLog.sessions.info("RealSessionsStore.loadDetail skipped reason=freshCache session=\(id, privacy: .public)"); return }; BotaplataLog.sessions.info("RealSessionsStore.loadDetail session=\(id, privacy: .public) reason=detailOpened"); let sp = BotaplataSignpost.begin("session detail load"); let existing = visibleDetail(id); details[id] = existing.map { .refreshing($0) } ?? .loading; detailGenerations[id, default: 0] += 1; let g = detailGenerations[id]!
        let task = Task { [repository, authSession] in do { let detail = try await Self.authorized(authSession: authSession) { try await repository.fetchSessionDetail(id: id, accessToken: $0) }; try Task.checkCancellation(); await MainActor.run { if self.detailGenerations[id] == g { self.details[id] = .loaded(detail); self.detailLoadedAt[id] = Date() } } } catch is CancellationError {} catch AuthenticationError.accessTokenExpired { await MainActor.run {
                self.appState.transition(to: .expired)
                return
            } } catch AuthenticationError.deviceRevoked { await MainActor.run {
                self.appState.transition(to: .revoked)
