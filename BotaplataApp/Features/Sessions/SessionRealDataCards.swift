...
struct HealthFreshnessCard: View { let session: SessionDetail; var cached = false
    var body: some View { PremiumCard { VStack(alignment: .leading, spacing: BotaplataSpacing.sm) { Text("État de Botaplata").font(BotaplataTypography.cardTitle); PremiumKeyValueRow(label: "Surveillance", value: session.runtimeHealth == .healthy ? "Fonctionne normalement" : session.runtimeHealth.label, monospaced: false); PremiumKeyValueRow(label: "Données", value: cached ? "Dernier état connu" : SessionFreshnessPresentation.text(for: session.freshness), monospaced: false); if let date = session.decision.createdAt { PremiumKeyValueRow(label: "Dernière décision", value: SessionFreshnessPresentation.relativeText(for: DataFreshness(status: .fresh, updatedAt: date, source: .backend)), monospaced: false) }; if let errors = session.monitoringConsecutiveErrors, errors > 0 { Text("\(errors) erreurs de surveillance consécutives").foregroundStyle(BotaplataColors.warning) } } }.accessibilityElement(children: .combine) }
}
