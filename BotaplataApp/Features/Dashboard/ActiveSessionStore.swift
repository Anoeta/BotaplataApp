...
final class ActiveSessionStore {
    ...
    func loadCacheThenRefresh() async { let signpost = BotaplataSignpost.begin("cache read"); if let cached = await cache.load() { BotaplataLog.dashboard.info("DashboardStore.cache status=hit"); content = .loadedFromCache(cached); latestGeneratedAt = cached.generatedAt } else { BotaplataLog.dashboard.info("DashboardStore.cache status=miss") }; BotaplataSignpost.end("cache read", id: signpost); await refresh() }
    func refresh() async {
        ...
        BotaplataLog.dashboard.info("DashboardStore.refresh reason=networkRefresh")
