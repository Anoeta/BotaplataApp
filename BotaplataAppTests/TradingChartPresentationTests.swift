...
final class TradingChartPresentationTests: XCTestCase {
    ...
    func testCandleWidthBounds() { XCTAssertEqual(TradingChartPresentation.candleWidth(availableWidth: 0, candleCount: 0), 3); XCTAssertEqual(TradingChartPresentation.candleWidth(availableWidth: 320, candleCount: 1), 10); XCTAssertGreaterThanOrEqual(TradingChartPresentation.candleWidth(availableWidth: 320, candleCount: 672), 1); XCTAssertLessThanOrEqual(TradingChartPresentation.candleWidth(availableWidth: 320, candleCount: 60), 10) }
    ...
    func testRealChartLayoutConstantsAreBounded() {
        XCTAssertEqual(TradingChartPresentation.priceChartHeight, 268)
        XCTAssertEqual(TradingChartPresentation.volumeChartHeight, 72)
    }

    func testCandleWidthByObservedRanges() {
        let width: CGFloat = 320
        XCTAssertEqual(TradingChartPresentation.candleWidth(availableWidth: width, candleCount: 11), 12)
        XCTAssertGreaterThan(TradingChartPresentation.candleWidth(availableWidth: width, candleCount: 71), TradingChartPresentation.candleWidth(availableWidth: width, candleCount: 287))
        XCTAssertGreaterThan(TradingChartPresentation.candleWidth(availableWidth: width, candleCount: 287), TradingChartPresentation.candleWidth(availableWidth: width, candleCount: 672))
        XCTAssertGreaterThanOrEqual(TradingChartPresentation.candleWidth(availableWidth: width, candleCount: 672), 1)
    }

    func testXAxisDesiredCountsForObservedRanges() {
        XCTAssertEqual(TradingChartPresentation.xAxisDesiredCount(for: .oneHour, candleCount: 11), 3)
        XCTAssertEqual(TradingChartPresentation.xAxisDesiredCount(for: .sixHours, candleCount: 71), 4)
        XCTAssertEqual(TradingChartPresentation.xAxisDesiredCount(for: .oneDay, candleCount: 287), 5)
        XCTAssertEqual(TradingChartPresentation.xAxisDesiredCount(for: .sevenDays, candleCount: 672), 4)
    }

    func testIndicatorSeriesPresentAndAbsent() {
        let empty = RealTradingChartRenderModel.make(chart: FakeChartRepository.chart(candles: makeTradingCandles(count: 11)))
        XCTAssertFalse(empty.hasVWAP)
        XCTAssertFalse(empty.hasEMA200)
        XCTAssertFalse(empty.hasBollinger)
        var candles = makeTradingCandles(count: 71)
        candles[0] = TradingCandle(id: "i0", openTime: Date(timeIntervalSince1970: 0), closeTime: Date(timeIntervalSince1970: 60), isClosed: true, open: 1, high: 2, low: 1, close: 2, volume: 10, vwap: 1.5, ema200: 1.4, bollingerUpper: 2.2, bollingerMiddle: 1.6, bollingerLower: 0.8)
        let present = RealTradingChartRenderModel.make(chart: FakeChartRepository.chart(candles: candles))
        XCTAssertTrue(present.hasVWAP)
        XCTAssertTrue(present.hasEMA200)
        XCTAssertTrue(present.hasBollinger)
        XCTAssertTrue(present.hasVolume)
    }
